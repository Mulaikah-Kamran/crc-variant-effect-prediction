...
## 10. Classical baseline: BLOSUM62

Neither the hotspot check nor the ClinVar cross-reference answers a more basic question: does ESM2's score provide anything beyond what a simple, non-learned substitution matrix would already tell you? BLOSUM62 (Henikoff & Henikoff, 1992) is added as exactly that — a classical, well-established pairwise amino-acid substitution score, computed for the same 259 frozen variants already scored by ESM2.

**What BLOSUM62 measures and doesn't**: a higher score means a substitution that occurs more often between evolutionarily related sequences (more "tolerated" in that specific sense); a lower or negative score means a rarer, less-tolerated substitution. It is a log-odds score derived from observed substitution frequencies across aligned protein blocks — not a probability, not a pathogenicity score, not a functional measurement, and not specific to cancer or any disease context. It does carry real evolutionary information in that sense, but that information is generic: the same fixed score applies to a given amino acid pair everywhere, regardless of which protein or which position it occurs at. It has no way to know that position 600 of BRAF behaves differently from position 12 of KRAS. ESM2's distinguishing feature is exactly that position-specific, protein-specific context — the same substitution can score very differently in ESM2 depending on where it falls and what surrounds it, which is what makes the comparison meaningful: it asks how much of ESM2's signal is explained by generic substitution behavior alone, versus how much comes from context BLOSUM62 has no access to.

**Why this comparison, not a performance benchmark**: there is no ground-truth functional label in this project to benchmark either method against, and this comparison does not attempt to manufacture one. It characterizes *relationship*, not *accuracy*. A strong positive correlation between the two scores would indicate ESM2's rankings broadly recapitulate classical substitution preferences for these variants; a weak correlation indicates ESM2 is capturing something substitution identity alone does not — without implying that either behavior makes ESM2 better or worse.

**Implementation**: the standard BLOSUM62 matrix is included directly (not via an added dependency), defined once as an upper triangle and mirrored programmatically so symmetry is guaranteed by construction; it is checked in the test suite against independently-known reference values (e.g., tryptophan's self-substitution score of 11, cysteine's of 9) and for full 20×20 symmetry, rather than only for absence of a runtime error. Spearman rank correlation (via `scipy.stats.spearmanr`) is used rather than Pearson, since there's no reason to expect a linear relationship between the two scoring systems, only a monotonic one if related at all.

**Result**: overall Spearman ρ = 0.29 (n = 259, p < 0.001) — a real but weak-to-moderate positive correlation. Per-gene results vary considerably and are reported in full rather than summarized to one number: BRAF shows a strong correlation (ρ = 0.90, n = 8) despite its small sample; KRAS shows a moderate, significant correlation (ρ = 0.60, n = 21); several other genes show weak or statistically non-significant correlations. Genes with fewer than 5 unique variants are reported with the correlation explicitly withheld as not meaningful at that sample size, rather than computed and included anyway.

**Interpretation, stated at the level the evidence supports**: the moderate overall correlation suggests ESM2's scores are related to, but not redundant with, classical substitution chemistry — consistent with (not proof of) the model using sequence context beyond simple amino-acid identity. The per-gene variation, including BRAF's notably strong correlation despite having the same general substitution-matrix baseline as every other gene, is itself worth noting as an open question rather than a conclusion: this analysis does not investigate why BRAF's relationship is stronger than KRAS's or TP53's, only that it is.
